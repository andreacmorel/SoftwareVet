...
// Array que acumulará los errores de validación del formulario
$erroresCampos = [];

// Solo procesa si el formulario fue enviado mediante el botón "btnGuardar"
if (!empty($_POST['btnGuardar'])) {

    // Recoge y limpia espacios al inicio y al final del nombre ingresado
    $nombre_perfil = trim($_POST['nombre_perfil'] ?? '');

    // --- Validaciones del campo "nombre_perfil" ---
    ...
    // Si no hay errores de formato, verifica que el nombre no esté ya registrado
    if (empty($erroresCampos)) {

        // Escapa el valor para usarlo de forma segura en la consulta SQL
        $nombreSeguro = $conexion->real_escape_string($nombre_perfil);

        // Busca si ya existe un perfil activo con el mismo nombre
        ...
        // Si encontró al menos un registro, el nombre ya está en uso
    ...
    // Si todas las validaciones pasaron, inserta el nuevo perfil en la BD
    if (empty($erroresCampos)) {

        // Vuelve a escapar por si $nombreSeguro fue redefinido en otro contexto
        $nombreSeguro = $conexion->real_escape_string($nombre_perfil);

        // Inserta el perfil con estado activo (1)
        $conexion->query("
            INSERT INTO perfil (nombre_perfil, estado)
            VALUES ('$nombreSeguro', 1)
        ");

        // Redirige al listado con parámetro que indica que la operación fue exitosa
...
<div class="container-fluid">
    ...
    <div class="form-card">
        ...
        <form method="POST" novalidate>
            ...
            <!-- Acciones: cancelar a la izquierda, guardar a la derecha -->
            <div class="d-flex justify-content-between">
                ...
                <!-- name="btnGuardar" value="1" permite identificar este envío en el POST -->
